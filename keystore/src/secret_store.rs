...
use ckeys::{sign_schnorr, Address, Message, Public, SchnorrSignature, Secret};
...
pub trait SimpleSecretStore: Send + Sync {
    ...
    fn sign(&self, account: &Address, password: &str, message: &Message) -> Result<SchnorrSignature, Error>;
...
pub trait SecretStore: SimpleSecretStore {
    ...
    fn sign_with_secret(&self, secret: &OpaqueSecret, message: &Message) -> Result<SchnorrSignature, Error> {
        Ok(sign_schnorr(&secret.0.into(), message)?)
    }
