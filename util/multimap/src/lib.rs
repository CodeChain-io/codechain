...
pub struct MultiMap<K, V> where K: Ord + Eq + Hash, V: Eq + Hash {
	backing: BTreeMap<K, HashSet<V>>,
}

impl<K, V> MultiMap<K, V> where K: Ord + Eq + Hash, V : Eq + Hash {
	...
	pub fn remove(&mut self, key: &K, value: &V) -> bool {
		if let Some(values) = self.backing.get_mut(key) {
			...
			if value != values.iter().next().expect("We know there is only one element in collection, tested above; qed") {
...
impl<K, V> Deref for MultiMap<K, V> where K: Ord + Eq + Hash, V: Eq + Hash {
