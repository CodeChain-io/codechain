...
impl HashDB for ArchiveDB {
    fn keys(&self) -> HashMap<H256, i32> {
        let mut ret: HashMap<H256, i32> = self.backing
            .iter(self.column)
...
mod tests {
    ...
    fn insert_same_in_fork() {
        ...
        jdb.commit_batch(3, &blake256(b"1002a"), Some((1, blake256(b"1"))))
            .unwrap();
        jdb.commit_batch(4, &blake256(b"1003a"), Some((2, blake256(b"2"))))
            .unwrap();

        jdb.remove(&x);
        jdb.commit_batch(3, &blake256(b"1002b"), Some((1, blake256(b"1"))))
            .unwrap();
        let x = jdb.insert(b"X");
        jdb.commit_batch(4, &blake256(b"1003b"), Some((2, blake256(b"2"))))
            .unwrap();

        jdb.commit_batch(5, &blake256(b"1004a"), Some((3, blake256(b"1002a"))))
            .unwrap();
        jdb.commit_batch(6, &blake256(b"1005a"), Some((4, blake256(b"1003a"))))
            .unwrap();
    ...
    fn long_history() {
        ...
        jdb.commit_batch(2, &blake256(b"2"), None).unwrap();
        assert!(jdb.contains(&h));
        jdb.commit_batch(3, &blake256(b"3"), Some((0, blake256(b"0"))))
            .unwrap();
        assert!(jdb.contains(&h));
        jdb.commit_batch(4, &blake256(b"4"), Some((1, blake256(b"1"))))
            .unwrap();
        assert!(jdb.contains(&h));
    }
    ...
    fn complex() {
        ...
        jdb.commit_batch(1, &blake256(b"1"), Some((0, blake256(b"0"))))
            .unwrap();
        assert!(jdb.contains(&foo));
        assert!(jdb.contains(&bar));
        ...
        let foo = jdb.insert(b"foo");
        jdb.remove(&baz);
        jdb.commit_batch(2, &blake256(b"2"), Some((1, blake256(b"1"))))
            .unwrap();
        assert!(jdb.contains(&foo));
        assert!(jdb.contains(&baz));

        jdb.remove(&foo);
        jdb.commit_batch(3, &blake256(b"3"), Some((2, blake256(b"2"))))
            .unwrap();
        assert!(jdb.contains(&foo));

        jdb.commit_batch(4, &blake256(b"4"), Some((3, blake256(b"3"))))
            .unwrap();
    }
    ...
    fn fork() {
        ...
        jdb.remove(&foo);
        let baz = jdb.insert(b"baz");
        jdb.commit_batch(1, &blake256(b"1a"), Some((0, blake256(b"0"))))
            .unwrap();

        jdb.remove(&bar);
        jdb.commit_batch(1, &blake256(b"1b"), Some((0, blake256(b"0"))))
            .unwrap();
        ...
        jdb.commit_batch(2, &blake256(b"2b"), Some((1, blake256(b"1b"))))
            .unwrap();
        assert!(jdb.contains(&foo));
    }
    ...
    fn overwrite() {
        ...
        jdb.commit_batch(2, &blake256(b"2"), Some((1, blake256(b"1"))))
            .unwrap();
        assert!(jdb.contains(&foo));
        jdb.commit_batch(3, &blake256(b"2"), Some((0, blake256(b"2"))))
            .unwrap();
        assert!(jdb.contains(&foo));
    }
    ...
    fn fork_same_key() {
        ...
        jdb.commit_batch(2, &blake256(b"2a"), Some((1, blake256(b"1a"))))
            .unwrap();
        assert!(jdb.contains(&foo));
    }
    ...
    fn reopen_remove() {
...

        {
            ...
            jdb.commit_batch(4, &blake256(b"4"), Some((3, blake256(b"3"))))
                .unwrap();
            jdb.commit_batch(5, &blake256(b"5"), Some((4, blake256(b"4"))))
                .unwrap();
        }
    }
