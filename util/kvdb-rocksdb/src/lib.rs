...
use rocksdb::{BlockBasedOptions, Cache, Column, DBIterator, Direction, IteratorMode, Options, ReadOptions, Writable,
    ...
    pub fn write(&self, tr: DBTransaction) -> Result<()> {
        match *self.db.read() {
            Some(DBAndColumns { ref db, ref cfs }) => {
                ...
                check_for_corruption(&self.path, db.write_opt(batch, &self.write_opts)).map_err(Into::into)
            }
            None => Err("Database is closed".into()),
        }
    }
            ...
            Some(DBAndColumns { ref db, ref cfs }) => {
                ...
                let mut overlay_data = overlay
                    ...
                    .filter_map(|(k, v)| match *v {
                        KeyState::Insert(ref value) | KeyState::InsertCompressed(ref value) => Some((
