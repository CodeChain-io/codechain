...
impl Node {
    ...
    fn from_rlp(rlp: &[u8], db: &HashDB, storage: &mut NodeStorage) -> Self {
        match RlpNode::decoded(rlp) {
            ...
            RlpNode::Extension(key, cb) => Node::Extension(key.encoded(false), Self::inline_or_hash(cb, db, storage)),
...
impl<'a> TrieDBMut<'a> {
    ...
    pub fn new(db: &'a mut HashDB, root: &'a mut H256) -> Self {
        *root = BLAKE_NULL_RLP;
        let root_handle = NodeHandle::Hash(BLAKE_NULL_RLP);
        ...
        Ok(match node {
            ...
            Node::Leaf(encoded, stored_value) => {
                ...
                if cp == existing_key.len() && cp == partial.len() {
                    ...
                    *old_val = Some(stored_value);
                    ...
                    let branch_action = self.insert_inspector(branch, partial, value, old_val)?
                        .unwrap_node();
                    ...
                    let branch = self.insert_inspector(branch, partial.mid(cp), value, old_val)?
                        .unwrap_node();
                    ...
                    let augmented_low = self.insert_inspector(low, partial.mid(cp), value, old_val)?
                        .unwrap_node();
            ...
            Node::Extension(encoded, child_branch) => {
                ...
                if cp == 0 {
                    ...
                    // continue inserting.
                    let branch_action = self.insert_inspector(Node::Branch(children, None), partial, value, old_val)?
                        .unwrap_node();
                    InsertAction::Replace(branch_action)
                } else if cp == existing_key.len() {
                    trace!(target: "trie", "complete-prefix (cp={:?}): AUGMENT-AT-END", cp);
...
impl<'a> TrieMut for TrieDBMut<'a> {
    ...
    fn is_empty(&self) -> bool {
        match self.root_handle {
            NodeHandle::Hash(h) => h == BLAKE_NULL_RLP,
