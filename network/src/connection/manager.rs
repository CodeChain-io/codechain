...
impl Manager {
    ...
    fn register_token(
        ...
        let session = self.address_to_session
            .get(&address)
    ...
    pub fn accept(&mut self, _io: &IoContext<HandlerMessage>) -> IoHandlerResult<Option<ConnectionToken>> {
        match self.listener.accept() {
            Ok((stream, address)) => self.register_token(stream, &SocketAddr::from(address), true),
    ...
    pub fn connect(&mut self, address: &SocketAddr) -> IoHandlerResult<Option<ConnectionToken>> {
        match TcpStream::connect(address.into()) {
            Ok(stream) => self.register_token(stream, &address, false),
            Err(ref e) if e.kind() == io::ErrorKind::WouldBlock => Ok(None),
            Err(e) => Err(From::from(e)),
        }
    }
    ...
    pub fn register_stream(
        ...
        let connection = self.connections
            .get(token)
            .ok_or(Error::NotValidStream(token))?;
        event_loop.register(
            connection.stream(),
            reg,
    ...
    fn message(&self, io: &IoContext<HandlerMessage>, message: &HandlerMessage) -> IoHandlerResult<()> {
        match *message {
            ...
            HandlerMessage::RequestConnection(ref address, ref session) => {
                ...
                info!("Register session {:?}", session);
                let _ = manager.register_session(address.clone(), session.clone());
            ...
            HandlerMessage::RequestNegotiation {
                ...
                let mut connection = manager
                    .connections
                    .get_mut(node_id)
                    .ok_or(Error::NotValidNode(node_id))?;
            ...
            HandlerMessage::SetTimer {
                ...
                match timer.insert(extension_name.clone(), timer_id, false) {
                    ...
                    Err(TimerInfoError::DuplicatedTimerId) => {
                        self.client
                            .on_timer_set_denied(extension_name, timer_id, ExtensionError::DuplicatedTimerId);
                    }
                    Err(TimerInfoError::NoSpace) => {
                        self.client
                            .on_timer_set_denied(extension_name, timer_id, ExtensionError::NoMoreTimerToken);
                    }
                }
            }
            ...
            HandlerMessage::ClearTimer {
                ...
                let mut timer = self.timer.lock();
                let token = timer
                    .remove_by_info(extension_name.clone(), timer_id)
                    .expect("Unexpected timer id");
            ...
            ACCEPT_TOKEN => loop {
                ...
                if let Some(token) = manager.accept(io)? {
                    if let Err(err) = io.register_stream(token) {
                        info!(
                            "Cannot register stream for accepted connection({:?}) : {:?}",
                            token, err
                        );
                    }
                } else {
        ...
        match stream {
            ...
            FIRST_CONNECTION_TOKEN...LAST_CONNECTION_TOKEN => loop {
                ...
                let mut connection = manager
                    .connections
                    .get_mut(stream)
                    .ok_or(Error::NotValidStream(stream))?;
                if !connection.send()? {
                    return Ok(());
                }
            },
