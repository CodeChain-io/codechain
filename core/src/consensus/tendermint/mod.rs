...
impl Tendermint {
    ...
    pub fn new(our_params: TendermintParams, machine: CodeChainMachine) -> Result<Arc<Self>, Error> {
        ...
        let handler = TransitionHandler::new(
            Arc::downgrade(&engine) as Weak<ConsensusEngine<_>>,
            Box::new(our_params.timeouts),
        );
    ...
    fn is_signer_proposer(&self, bh: &H256) -> bool {
        let proposer = self.view_proposer(
            bh,
    ...
    fn is_height(&self, message: &ConsensusMessage) -> bool {
        message
            .vote_step
            .is_height(self.height.load(AtomicOrdering::SeqCst))
    }

    fn is_view(&self, message: &ConsensusMessage) -> bool {
        message.vote_step.is_view(
            self.height.load(AtomicOrdering::SeqCst),
            self.view.load(AtomicOrdering::SeqCst),
        )
    }
    ...
    fn is_authority(&self, address: &Address) -> bool {
        self.validators
            .contains(&*self.proposal_parent.read(), address)
    }
            ...
            Step::Precommit => {
                ...
                let block_hash = match *self.lock_change.read() {
                    Some(ref m) if self.is_view(m) && m.block_hash.is_some() => {
                        ...
                        self.last_lock
                            .store(m.vote_step.view, AtomicOrdering::SeqCst);
                        m.block_hash
                    }
                    _ => None,
                };
    ...
    fn generate_message(&self, block_hash: Option<BlockHash>) -> Option<Bytes> {
        ...
        match (
            ...
            self.sign(blake256(&vote_info)).map(Into::into),
        ) {
        ...
        if self.is_height(message) {
            let next_step = match *self.step.read() {
                ...
                Step::Precommit if self.has_enough_aligned_votes(message) => {
                    let bh = message
                        .block_hash
                        .expect("previous guard ensures is_some; qed");
...
impl ConsensusEngine<CodeChainMachine> for Tendermint {
    ...
    fn generate_seal(&self, block: &ExecutedBlock, _parent: &Header) -> Seal {
        ...
        if let Ok(signature) = self.sign(blake256(&vote_info)).map(Into::into) {
            ...
            self.votes.vote(
                ConsensusMessage::new(signature, height, view, Step::Propose, bh),
                *author,
            );
        ...
        if let Ok(proposal) = ConsensusMessage::new_proposal(header) {
            ...
            for rlp in UntrustedRlp::new(signatures_field).iter() {
                ...
                let address = match self.votes.get(&precommit) {
                    Some(a) => a,
                    None => public_to_address(&recover_ecdsa(
                        &precommit.signature.into(),
                        &precommit_hash,
                    )?),
                };
    ...
    fn handle_message(&self, rlp: &[u8]) -> Result<(), EngineError> {
        ...
        if !self.votes.is_old_or_known(&message) {
            ...
            if let Some(double) = self.votes.vote(message.clone(), sender) {
                ...
                self.validators
                    .report_malicious(&sender, height, height, ::rlp::encode(&double).into_vec());
    ...
    fn register_client(&self, client: Weak<EngineClient>) {
        if let Some(c) = client.upgrade() {
            self.height.store(
                c.chain_info().best_block_number as usize + 1,
    ...
    fn populate_from_parent(&self, header: &mut Header, parent: &Header) {
        ...
        let new_score = U256::from(U128::max_value())
            + consensus_view(parent)
                .expect("Header has been verified; qed")
                .into() - self.view.load(AtomicOrdering::SeqCst).into();
    ...
    fn is_proposal(&self, header: &Header) -> bool {
        ...
        let proposer = proposal
            .verify()
            .expect("block went through full verification; this Engine tries verify; qed");
    ...
    F: Fn(&ECDSASignature, &Message) -> Result<Address, Error> + Send + Sync,
{
    fn verify_light(&self, header: &Header) -> Result<(), Error> {
        ...
        for rlp in UntrustedRlp::new(header_signatures_field).iter() {
            ...
            if !self.subchain_validators
                .contains(header.parent_hash(), &address)
            {
                return Err(EngineError::NotAuthorized(address.to_owned()).into());
            }
