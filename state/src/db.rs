...
impl StateDB {
    ...
    pub fn new(db: Box<JournalDB>, cache_size: usize, custom_handlers: Vec<Arc<ActionHandler>>) -> StateDB {
        ...
        StateDB {
            ...
            shard_metadata_cache: Arc::new(Mutex::new(Cache {
                cache: LruCache::new(shard_metadata_cache_items),
                modifications: VecDeque::new(),
            })),
...
impl ShardBackend for StateDB {
    fn add_to_shard_metadata_cache(
        ...
        self.local_shard_metadata_cache.push(CacheQueueItem {
            address,
            item,
            modified,
        })
...
mod tests {
    ...
    fn asset_scheme_cache() {
        ...
        let asset_scheme =
            AssetScheme::new("A metadata for test asset_scheme".to_string(), amount, registrar, Vec::new());
