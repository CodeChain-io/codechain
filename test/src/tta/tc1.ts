...
(async () => {
    ...
    for (let k = 0; k < 4; k++) {
        tasks.push(
            (async function(nodeNum: number) {
                let index = 0;
                let sentCount = 0;
                while (true) {
                    const futureCnt = await sdks[
                        nodeNum
                    ].rpc.sendRpcRequest("mempool_getCurrentFuturueCount", [
                        null,
                        null
                    ]);
                    const txToSend = Math.min(
                        maxSend,
                        desiredMempoolSize - futureCnt[0]
                    );
                    if (txToSend > minSend) {
                        const txs = [];
                        for (let i = 0; i < txToSend; i++) {
                            txs.push(transactions[nodeNum][index]);
                            index += 1;
                        ...
                        await sdks[
                            nodeNum
                        ].rpc.sendRpcRequest("mempool_sendSignedTransactions", [
                            txs
                        ]);
                        sentCount += txToSend;
                        if (sentCount > 5000) {
                            ...
                            console.log(
                                `Txs sent for Node ${nodeNum}: ${sentCount}`
                            );
                            console.log(
                                `Mempool for Node ${nodeNum}: ${futureCnt}`
                            );
                            sentCount = 0;
                        }
                    } else {
    ...
    let total = 0;
    {
        ...
        while (total < numTransactions * 4) {
            const x = (await sdks[0].rpc.sendRpcRequest(
    ...
    while (true) {
        ...
        if (lastNum !== num) {
            ...
            for (let b = lastNum + 1; b <= num; b++) {
                blocks.push(
                    (await sdks[0].rpc.sendRpcRequest(
                        "chain_getHeaderAndTxCountByNumber",
                        [b]
                    ))!
                );
